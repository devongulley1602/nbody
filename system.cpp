...
class System {
	...
	public:
		...
		//The Barnes-Hutt theta criterion set by the system (typically 0.5) -> the ratio of the domain of an internal node to the distance it is from a body 
		double theta;	



		/*
	 	 * Builds the Barnes-Hutt tree to represent point body clusters
		 *
	   ...
	   */
		...
		void calculateForces(Body body, Body node,double (&F)[3]){
			...
			if(node.isExternal ||( (node.Domain()*node.Domain()< theta*node.distanceSquaredTo(body)) && &node!=&headNode) ){
